...
OPTIONS
  ...
  --uninstall            Remove the collectors. The user account is left alone.
  -h, --help             This.
HELPDOC
}
...
setup_atalaya_user() {
  ...
  # In stack's group, and in no other. That is the whole permission model: the
  # group can traverse the home and read the inventory, while secrets/ is 700
  # and stays out of reach. No sudo is granted, here or anywhere.
...
verify() {
  ...
  if id "$ATALAYA_USER" >/dev/null 2>&1; then
    ...
    if sudo -n -l -U "$ATALAYA_USER" 2>&1 | grep -q 'not allowed'; then
      ok "$ATALAYA_USER has no sudo"
    else
      fail "$ATALAYA_USER appears to have sudo rights"
      failures=$((failures + 1))
    fi
  fi
